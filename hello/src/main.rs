//! `/bin/hello` — minimal user binary that proves exec works
//! end-to-end. Runs out of the tarfs-served disk image; the console
//! `exec` builtin (or the FS smoke) reads the bytes and hands
...
/// return value is passed to `exit` — `42` keeps the wait_pid
...
pub extern "C" fn main() -> i32 {
    ...
    // Print argv. Each line is a separate serialln so
