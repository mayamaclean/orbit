...
use virtio_input::{Input, InputBacking, EVENT_SIZE, VIRTIO_INPUT_DEVICE_ID};
...
pub fn setup_virtio_input(kernel_pages: &mut KernelPages) -> bool {
    let Some(found) = virtio_probe::find(VIRTIO_INPUT_DEVICE_ID) else {
        info!("virtio-input: no device-id 18 slot present");
        return false;
    };
    let slot = found.slot;
    let mmio = found.mmio;
    info!("virtio-input: selected slot @{:#x} irq={}", slot.pa_base, slot.irq);
    ...
    unsafe { core::ptr::write_bytes(q_kva.as_mut_ptr::<u8>(), 0, QUEUE_PAGE_SIZE); }
    ...
    unsafe { core::ptr::write_bytes(arena_kva.as_mut_ptr::<u8>(), 0, QUEUE_PAGE_SIZE); }
...
fn virtio_input_handler(_src: u32) {
    let Some(dev) = input_dev() else { return };
    unsafe {
